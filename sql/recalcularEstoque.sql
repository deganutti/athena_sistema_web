-- Function: public.src_recalcular_estoque(integer, integer)

-- DROP FUNCTION public.src_recalcular_estoque(integer, integer);

CREATE OR REPLACE FUNCTION public.src_recalcular_estoque(
    p_codfilial integer,
    p_codart integer)
  RETURNS integer AS
$BODY$
          ...
          IF X.tipo_tabela = 1 THEN
             IF x.tipooperacao <> 3 THEN
                ...
                IF X.est_entregar > 0 THEN
                   ...
                   select COALESCE(SUM(entregue),0) INTO WEntregue 
                     from pedbaixa 
                    where dtmovto >= X.dt_movto 
                      AND hora > X.hora_movto 
                      and codart = p_codart 
                      and codfilial = p_codfilial ;

                   select COALESCE(SUM(entregue),0) 
                     INTO WEntregueTrans 
                     from pedtranbaixa 
                    where dtmovto >= X.dt_movto 
                      AND hora > X.hora_movto 
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.src_recalcular_estoque(integer, integer)
  OWNER TO postgres;
